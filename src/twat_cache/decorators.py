...
def mcache(
    ...
    *,  # Force remaining arguments to be keyword-only
    ...
    """
    logger.debug(
        f"Creating memory cache with maxsize={maxsize}, ttl={ttl}, policy={policy}"
    )
...
def bcache(
    ...
    *,  # Force remaining arguments to be keyword-only
    ...
    """
    logger.debug(
        f"Creating disk cache with folder={folder_name}, maxsize={maxsize}, ttl={ttl}, use_sql={use_sql}"
    )
...
def fcache(
    ...
    *,  # Force remaining arguments to be keyword-only
    ...
    """
    logger.debug(
        f"Creating file cache with folder={folder_name}, maxsize={maxsize}, ttl={ttl}, compress={compress}"
    )
...
def acache(
    ...
    """
    logger.debug(
        f"Creating async cache with maxsize={maxsize}, ttl={ttl}, policy={policy}"
    )
...
def ucache(
    ...
    *,  # Force remaining arguments to be keyword-only
    ...
    def decorator(func: Callable[P, R]) -> Callable[P, R]:
        ...
        backend = preferred_engine or _select_best_backend(
            is_async=is_async, needs_disk=needs_disk
        )
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            try:
                ...
                logger.warning(
                    f"Cache error in {func.__name__}: {e}. Executing without cache."
                )
